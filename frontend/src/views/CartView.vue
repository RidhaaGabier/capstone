  ...
  <div>
    <section class="overflow-x-hidden hero m-0 align-content-sm-start">
      <div class="hero-inner text-center">
        ...
        <a href="#" class="btn shadow__btn mb-2"
          >Shop<i class="fa-regular fa-greater-than">Decor</i></a
        >
      </div>
    </section>
    <div class="checkout" v-if="cart.length > 0">
          ...
          <tbody>
            <tr v-for="item in cart" :key="item.product.ProdID">
              ...
              <td>
                ...
                <button @click="AddItem(item)" class="btn btn-sm btn-secondary">
                  +
                </button>
              </td>
              ...
              <td>
                <button
                  @click="deleteProduct(item.product.ProdID)"
      ...
      <div class="d-flex justify-content-end w-100" v-if="cart.length > 0">
        ...
        <div id="grandTotal" class="text-black">
          Grand Total: R {{ grandTotal }}
        </div>
      </div>

      <form class="container" @submit="checkout" >
  ...
  methods: {
    ...
    deleteProduct(productId) {
      ...
      const updatedCart = this.cart.filter(
        (item) => item.product.ProdID !== productId
      );
